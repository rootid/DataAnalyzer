 ...
 * /1.5.0/user/src/com/google/gwt/user/server/rpc/
 * RemoteServiceServlet.java
 * This class is used for converting the rpcdata to http payload
 */
...
public class DataAnalyzerRemoteServlet extends HttpServlet implements SerializationPolicyProvider{
	...
	public DataAnalyzerRemoteServlet() {
	}
	...
	public final void doPost(HttpServletRequest request,
		...
		try {
			...
			// result.
			//
			String responsePayload = processCall(request, response,
					requestPayload);
	 ...
	 */
	public String processCall(HttpServletRequest request, HttpServletResponse response,
		...
		try {
			...
			Class<?>[] rpcMethodParamTypes = rpcMethod.getParameterTypes();
			Object[] rcpMethodParms = rpcRequest.getParameters();
			Class<?>[] serviceMethodParamType = new Class<?>[rpcMethodParamTypes.length + 2];
			Object[] serviceMethodParams = new Object[rcpMethodParms.length + 2];
			serviceMethodParamType[0] = HttpServletRequest.class;
			serviceMethodParamType[1] = HttpServletResponse.class;
			serviceMethodParams[0] = request;
			serviceMethodParams[1] = response;
			for (int i = 2; i < serviceMethodParamType.length; i++) {
				serviceMethodParamType[i] = rpcMethodParamTypes[i - 2];
				serviceMethodParams[i] = rcpMethodParms[i - 2];
			}
			Method serviceMethod = null;
			try {
				serviceMethod = this.getClass().getMethod(rpcMethod.getName(),
						serviceMethodParamType);
			} catch (SecurityException e) {
				throw new IncompatibleRemoteServiceException(e.getMessage());
			} catch (NoSuchMethodException e) {
				throw new IncompatibleRemoteServiceException(e.getMessage());
			}

			return RPC.invokeAndEncodeResponse(this, serviceMethod,
					serviceMethodParams, rpcRequest.getSerializationPolicy());
