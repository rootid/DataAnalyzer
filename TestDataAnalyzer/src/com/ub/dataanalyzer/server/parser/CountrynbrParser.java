...
/**
 * This class performs parsing to extract information about country neighbors.
 * @author vikram
 *
 */

public class CountrynbrParser extends Parser {
	

	public CountrynbrParser(String inputResponse) {
		super(inputResponse);
		this.inputStream = new ByteArrayInputStream(inputResponse.getBytes());;
		//fileName = inputResponse;
	}
...
//			for it to pick-up the correct encoding. 
	...
	class XmlParserHandler extends DefaultHandler {
		...
		public void startElement(String namespaceURI, String localName, 
			...
			for (String str : keys) {
				if(str.equalsIgnoreCase(qName) && isRootNode) {
					countForObject++;
					elementName = str;
					found = true;
					break;
			...
			if (!isRootNode) {
				if(qName.equalsIgnoreCase(keys[0])) {
					isRootNode = true;
				}
			}
		}
